...
fn generate_combinations_recursive(
    current: &mut CommandInstructions,
    index: usize,
    fields: &[Vec<(String, Box<dyn Fn(&mut CommandInstructions, String)>)>],
    rules: &Rules,
    valid_combinations: &mut Vec<CommandInstructions>,
) {
    if index == fields.len() {
...
async fn test_down_command_transposition() {
    ...
    let rules_function = Rules {
        ...
        command_targets: vec![
            CommandTarget::ClientKey("some_client".to_string()),
            CommandTarget::Host,
        ],
        response_targets: vec![
            Some(ResponseTarget::Origin),
            Some(ResponseTarget::Host),
            Some(ResponseTarget::ClientKey("some_client".to_string())),
        ],
        command_statuses: vec![CommandStatus::Success, CommandStatus::Failure],
        command_origins: vec![
            CommandOrigin::Host,
            CommandOrigin::ClientKey("some_client".to_string()),
        ],
    ...
    let fields_function: Vec<Vec<(String, Box<dyn Fn(&mut CommandInstructions, String)>)>> = vec![
        vec![(
            "Function".to_string(),
            Box::new(|c: &mut CommandInstructions, _| c.mode = CommandMode::Function),
        )],
        vec![
            ...
            (
                ...
                Box::new(|c: &mut CommandInstructions, _| {
                    c.command_type = CommandType::ExternalFunction
                }),
            ),
        ],
        vec![
            (
                "ClientKey(some_client)".to_string(),
                Box::new(|c: &mut CommandInstructions, v| c.target = CommandTarget::ClientKey(v)),
            ),
            (
                "Host".to_string(),
                Box::new(|c: &mut CommandInstructions, _| c.target = CommandTarget::Host),
            ),
        ],
        vec![
            (
                "Origin".to_string(),
                Box::new(|c: &mut CommandInstructions, _| {
                    c.response_target = Some(ResponseTarget::Origin)
                }),
            ),
            (
                "Host".to_string(),
                Box::new(|c: &mut CommandInstructions, _| {
                    c.response_target = Some(ResponseTarget::Host)
                }),
            ),
            (
                "ClientKey(some_client)".to_string(),
                Box::new(|c: &mut CommandInstructions, v| {
                    c.response_target = Some(ResponseTarget::ClientKey(v))
                }),
            ),
        ],
        vec![
            (
                "Success".to_string(),
                Box::new(|c: &mut CommandInstructions, _| c.status = CommandStatus::Success),
            ),
            (
                "Failure".to_string(),
                Box::new(|c: &mut CommandInstructions, _| c.status = CommandStatus::Failure),
            ),
        ],
        vec![
            ...
            (
                "ExternalFunction".to_string(),
                Box::new(|c: &mut CommandInstructions, _| {
                    c.response_type = Some(ResponseType::ExternalFunction)
                }),
            ),
        ],
        vec![
            (
                "Host".to_string(),
                Box::new(|c: &mut CommandInstructions, _| c.origin = CommandOrigin::Host),
            ),
            (
                "ClientKey(some_client)".to_string(),
                Box::new(|c: &mut CommandInstructions, v| c.origin = CommandOrigin::ClientKey(v)),
            ),
        ],
    ];
    ...
    let fields_response: Vec<Vec<(String, Box<dyn Fn(&mut CommandInstructions, String)>)>> = vec![
        vec![(
            "Response".to_string(),
            Box::new(|c: &mut CommandInstructions, _| c.mode = CommandMode::Response),
        )],
        vec![
            (
                "DirectFunction".to_string(),
                Box::new(|c: &mut CommandInstructions, _| {
                    c.command_type = CommandType::DirectFunction
                }),
            ),
            (
                "ExternalFunction".to_string(),
        ...
        vec![(
            String::new(),
            Box::new(|c: &mut CommandInstructions, _| c.response_target = None),
        )],
        vec![
            (
                "Success".to_string(),
        ...
        vec![
            (
                "DirectFunction".to_string(),
                Box::new(|c: &mut CommandInstructions, _| {
                    c.response_type = Some(ResponseType::DirectFunction)
                }),
            ),
            (
                "ExternalFunction".to_string(),
    ...
    ); // Generate the commands possible combinations
    generate_combinations_recursive(
        &mut initial_command,
        0,
        &fields_response,
    ...
    println!(
        "Generated {} valid CommandInstructions instances.",
    ...
    for (i, instruction) in valid_combinations.iter().enumerate() {
        ...
        println!(
            "\n{} responses and {} failed",
            total_responses, total_failures
        );
