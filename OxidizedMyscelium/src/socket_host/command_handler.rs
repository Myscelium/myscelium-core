...
pub async fn handle_redirect_error(
    e: RedirectError,
    client_key: String,
    parity_id: &String,
) -> Command {
    ...
    match e {
        ...
        RedirectError::ClientIsNotFullyInitialized(client) => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                format!("Client \"{client}\" is not fully initialized!")
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::ClientIsNotRunning(client) => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                format!("Client \"{client}\" is not running!")
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::CanNotRedirectFromOriginToHost => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                "Can't redirect from host to origin, this is a Origin to Host direct case!"
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::CanNotRedirectFromHostToOrigin => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                "Can't redirect from origin to host, this is a Origin to Host direct case!"
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::TargetDoesntExists(target) => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                format!("Target client \"{target}\" doesn’t exist!")
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::HandlerDoesntExist(handler) => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                format!("Handler \"{handler}\" doesn’t exist on the target client!")
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::ResponseHandlerDoesntExist(resp_handler) => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                format!("Response handler \"{resp_handler}\" doesn’t exist on the target client!")
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::CantScheduleCommandsToItself => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                "The origin client cannot schedule commands to itself."
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::HostCantSendResponseToItself => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                "The host client cannot send a response to itself."
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
        ...
        RedirectError::TargetCantSendResponseToItself => {
            let command: Command = create_error_command_response!(
                client_key.clone(),
                parity_id,
                "The target client cannot send a response to itself."
            );
            logger.debug(format!("Sending back: {:?}", &command)).await;
            command
        }
...
pub async fn redirect_commands_processing(
    command: &Command,
    target: &String,
    ...
    logger
        .debug(format!("Redirecting command to target: {}", target))
        .await;

    let command_instructions_to_schedule: CommandInstructions = match handle_redirect(
    ...
    .await
    {
        Ok(cmi) => cmi,
        Err(e) => {
            return vec![CommandVariant::Command(
                handle_redirect_error(e, command.client_key.clone(), &command.parity_id.clone())
                    .await,
            )];
        }
    };
    ...
    logger
        .debug("Command is in command patterns!".to_string())
        .await;
...
pub async fn host_commands_processing(command: &Command) -> Command {
    ...
    logger
        .debug("Command is in command patterns!".to_string())
        .await;
    let command_already_processed: bool =
        match enhanced_buffer::buffer_up_manager::check_if_parity_id_is_registered(
            command.parity_id.clone(),
            command.client_key.clone(),
        )
        .await
        {
            ...
            Err(e) => {
                let command: Command = create_error_command_response!(
                    ...
                    command.parity_id,
                    format!(
                        "Error trying to check if parity id is registered: {:?}",
                        &command
                    )
                );
                logger
    ...
    if command_already_processed {
        logger
            ...
            .await; // TODO >>> Change this error it's incorrect!
        let response = match get_response(command.clone()).await {
            ...
            Err(e) => {
                ...
                logger
                    ...
                    .await;
                create_error_command_response!(
                    command.client_key.clone(),
                    command.parity_id,
                    format!(
                        "Error trying get a response for the command: {:?}, the error was: {:?}",
                        &command, e
                    )
                )
            }
    ...
    response = match handle_common_function(&command).await {
        ...
        Err(e) => {
            ...
            create_error_command_response!(
                ...
                command.parity_id,
                format!(
                    "Error trying handle a common function command: {:?}, the error was: {:?}",
                    &command, e
                )
            )
        }
