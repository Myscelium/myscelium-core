...
pub async fn handle_direct_function(client_key: &String, activation_key: &String, command: CommandInstructions, command_id: Option<u32>) -> ProcessResult {
    ...
    match activation_key.as_str() {
        "get_registered_commands" => {
            logger.info(format!("Receive get_registered_commands in host!")).await;
        ...
        "update_client_commands_ref" => {
            ...
            logger.info(format!("Receive update_client_commands_ref in host!")).await;
        ...
        "restrictive_update_client_commands_ref" => {
            logger.info(format!("Receive restrictive_update_client_commands_ref in host!")).await;
        ...
        "add_client" => {
            ...
            logger.info(format!("Successfully add a client: {}!", new_client.client_key)).await;
        ...
        "update_client" => {
            ...
            logger.debug("Receive a update client inner command!".to_string()).await;

            if !command.kwargs.contains_key("actual_client_key") {
                logger.warn("Error! Callback response kwargs don't have actual_client_key kwarg!".to_string()).await;
            ...
            if !command.kwargs.contains_key("updated_client") {
                logger.warn("ERROR, Error! Callback response kwargs don't have update_client kwarg!".to_string()).await;
            ...
            let new_client = match command.kwargs.get("updated_client") {
                ...
                None => {
                    logger.warn(format!("Error! Kwargs doesn't have the `updated_client` kwargs")).await;
            ...
            let old_client = match Client::get_by_key(&actual_client_key.to_string()).await {
                Ok(old_c) => old_c,
                Err(e) => {
                    return ProcessResult::Error(format!("Client Error trying to get client by key: {:?}", e));
                },
            };
            ...
            match old_client.update_to(&new_client).await {
                ...
                Ok(_) => {
                    ...
                    logger.info(format!("Successfully executed the function: {} and remove client: {}!", activation_key, old_client.client_key)).await;
                ...
                Err(e) => match e {
                    ClientError::ClientDoesNotExist(e) => {
                        logger.warn(format!("Error! Can't Update client because client {} Don't exist!", e)).await;
                        return ProcessResult::Error(format!("Error! Can't Update client because client {} Don't exist!", e));
                    },
                    _ => {
                        logger.warn("Error! Can Update client because a unexpected error!".to_string()).await;
        ...
        "remove_client" => {
            ...
            match result {
                Err(e) => match e {
                    ClientError::ClientDoesNotExist(e) => {
                        logger.warn(format!("Error! Can't Remove client because client {} Don't exist!", e)).await;
                        return ProcessResult::Error(format!("Error! Can't Remove client because client {} Don't exist!", e));
                    },
                    _ => {
                        logger.warn("Error! Can Remove client because a unexpected error!".to_string()).await;
                ...
                Ok(_) => {
                    ...
                    logger.info(format!("Successfully executed the function: {} and remove client: {}!", activation_key, client_key)).await;
