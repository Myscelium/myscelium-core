...
pub async fn set_host_log_level(log_level: String) {
    {
        let mut current_log_level = HOST_LOG_LEVEL.lock().await;
...
impl Logger {
    pub async fn new(log_level: String, section: &str) -> Self {
        ...
        let node_name: String = HOST_NODE_NAME.lock().await.clone();
