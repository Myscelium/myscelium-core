...
use crate::common::enhanced_buffer::history::register::register::{
    initialize_buffer_history, write_to_file,
};
...
impl BufferHistory {
    ...
    pub fn log_add_operation(
        ...
        if let Some(id) = id {
            to_write = format!(
                "[{}][{}][{}][{}][{}][ADD] - {}",
                ts_stamp, self.buffer_type, client_key, unique_key, id, operation
            );
        } else {
            to_write = format!(
                "[{}][{}][{}][{}][{}][ADD] - {}",
                ts_stamp, self.buffer_type, client_key, unique_key, "", operation
            );
        }
