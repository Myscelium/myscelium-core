...
fn test_convert_to_resulttype_map() {
    ...
    nested_map.insert("nested_key".to_string(), ResultType::Str("nested_value".to_string()));
...
fn test_value_to_resulttype() {
    ...
    assert_eq!(value_to_resulttype(&json!({"key": "value"})), Ok(ResultType::Map(map.clone())));

    let list = vec![ResultType::Str("value".to_string())];
    assert_eq!(value_to_resulttype(&json!(["value"])), Ok(ResultType::List(list)));

    assert!(matches!(value_to_resulttype(&json!(42.0)), Ok(ResultType::Float(42.0))));
    assert!(matches!(value_to_resulttype(&json!(42.0)), Ok(ResultType::Float(42.0))));
    assert!(matches!(value_to_resulttype(&json!({"key": "value"})), Ok(ResultType::Map(_))));
    assert!(matches!(value_to_resulttype(&json!(["value"])), Ok(ResultType::List(_))));
}
...
fn test_resulttype_to_value() {
    ...
    let mut map = HashMap::new();
    map.insert("key".to_string(), ResultType::Str("value".to_string()));
    assert_eq!(resulttype_to_value(&ResultType::Map(map.clone())), json!({"key": "value"}));

    let list = vec![ResultType::Str("value".to_string())];
    assert_eq!(resulttype_to_value(&ResultType::List(list.clone())), json!(["value"]));
}
...
fn test_convert_json_map_to_hash_map() {
    let json_map: Map<String, Value> = serde_json::from_str(r#"{"key1": "value1", "key2": 42}"#).unwrap();
