...
pub fn logs_register_initialize_table(loggs_storage_path: String) {
    ...
    with_connection!(LOGS_REGISTERS_POOL, |conn: &rusqlite::Connection| {
        ...
        match result {
            ...
            Err(e) => {
                eprintln!(
                    "An error occurred while scheduling the command in the ClientLogs table: {}",
                    e
                );
            }
        }
...
pub fn registry_log(
    ...
    with_connection!(LOGS_REGISTERS_POOL, |conn: &rusqlite::Connection| {
        ...
        match result {
            ...
            Err(e) => {
                eprintln!(
                    "An error occurred while inserting the Log in the table ClientLogs: {}",
                    e
                );
            }
        };
...
pub fn remove_log_by_id(log_id: u32) {
    with_connection!(LOGS_REGISTERS_POOL, |conn: &rusqlite::Connection| {
        ...
        match result {
            ...
            Err(e) => {
                eprintln!(
                    "An error occurred while deleting the Log: {} from ClientLogs table: {}",
                    log_id, e
                );
            }
        };
