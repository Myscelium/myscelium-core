...
use crate::{
    CLIENT_BUFFER_ACTIVATION_CONTROLLER, CLIENT_IS_SYNC, CLIENT_NODE_CONFIGS, MEDIAN_CON_RESP_TIME,
};
...
pub fn initialize_client(address: String) -> Option<String> {
    ...
    loop {
        ...
        if !(up_schdule_len > 0) {
            ...
            logger.debug(format!(
                "Nothing in schedule to send to host, so sending ping!"
            ));
            if let Some(down_command) = option_down_command {
                ...
                logger.debug(format!(
                    "[Socket] - No command received in ping, skipping.."
                ));
            }
                    ...
                    received = match send(&mut stream, &command_to_request) {
                        ...
                        Err(e) => {
                            ...
                            match e {
                                ...
                                StreamError::WriteError(e) => {
                                    logger
                                        .debug(format!("[CLIENT][SOCKET][WRITE ERROR] - {:?}", e));
                                    logger.debug(format!(
                                        "[CLIENT][SOCKET][CLOSE CONNECTION] - {}",
                                        &client_key
                                    ));
                                ...
                                StreamError::WriteSizeError(e) => {
                                    logger.debug(format!(
                                        "[CLIENT][SOCKET][WRITE SIZE ERROR] - {:?}",
                                        e
                                    ));
                                    logger.debug(format!(
                                        "[CLIENT][SOCKET][CLOSE CONNECTION] - {}",
