        ...
        match response.command_type() {
            ...
            CommandType::SpecialFunction => {
                ...
                if response.command.actf == "C207".to_string() {
                    logger.debug(format!("Receive ping confirmation")).await;
                    continue;
                }
...
impl fmt::Display for TaskError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TaskError::UnexpectedError(msg) => write!(f, "unexpected error: {msg}"),
            TaskError::ClientIsNotRunning => write!(f, "client is not running"),
        }
    }
}

async fn load_buffer_and_send(tx_outbound: Sender<String>) -> Result<(), TaskError> {
    let logger = acquire_logger!("Core");
    ...
    if up_schdule_len == 0 {
        return Ok(()); // TODO >>> Instead of looping to find data to send, when have the IPCNS working start to use tx mpsc reactive activation to send data
    }
    ...
    for up_command in up_schedule {
        logger.debug(format!("processing command: {}", index)).await;
        ...
        tx_outbound
            .send(command_response_json)
            .await
            .map_err(|e| TaskError::UnexpectedError(format!("Error trying to send a command response json to the tx_outbound (Sender) channel, the error was: {:?}", e)))?;
...
pub async fn initialize_client(address: String, shutdown: Arc<Notify>) -> Option<String> {
    ...
    let recv_task = tasks.spawn(async move {
        ...
        select! {
            ...
            _ = recv_shutdown.notified() => {
                // Shutdown requested
                logger.info("Receiver task shutting down".to_string()).await;
            }
        }
    });
    ...
    let send_task = tasks.spawn(async move {
        ...
        select! {
            ...
            _ = send_shutdown.notified() => {
                logger.info("Sender task shutting down".to_string()).await;
            }
        }
    });
    ...
    tasks.spawn(async move {
        ...
        interval.tick().await; // Fire the first tick immediately
        println!("Heartbeat task started"); // TODO >>> Also add the logger to this ones
        loop {
            tokio::select! {
                _ = interval.tick() => {
                    // send heartbeat
    ...
    let tx = tx_outbound.clone();
    tasks.spawn(async move {
        let mut interval = tokio::time::interval(Duration::from_millis(50));
        interval.tick().await;
        loop {
            tokio::select! {
                _ = loader_shutdown.notified() => break,
                _ = interval.tick() => {
                    if let Err(e) = load_buffer_and_send(tx.clone()).await {
                        eprintln!("buffer loader failed: {e}");
                        break;
                    }
                }
            }
